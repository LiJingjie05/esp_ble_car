...
void app_main()
{
    ...
    while (1)
    {
        printf("Encoder 1 count: %d\n", get_encoder_val(&encoder1));
        printf("Encoder 2 count: %d\n", get_encoder_val(&encoder2));
        led_on();
        set_motor_pwm(&motor1, 0, 128);
        set_motor_pwm(&motor2, 128, 0);
        vTaskDelay(1000 / portTICK_PERIOD_MS); // 延迟 1 秒
        set_motor_pwm(&motor1, 128, 0);
        set_motor_pwm(&motor2, 0, 128);
        led_off();
        vTaskDelay(1000 / portTICK_PERIOD_MS); // 延迟 1 秒
    }
}
